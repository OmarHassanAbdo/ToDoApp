...
import com.example.todo.security.requests.Login;
import com.example.todo.security.requests.Signup;
import com.example.todo.service.EmailService;
import com.example.todo.service.servicesInterfaces.PasswordResetTokenService;
...
public class AuthController {
    ...
    @PostMapping("/signup")
    public ApiResponse<User> signup(@RequestBody Signup request){
    if(userRepository.existsByEmail(request.getEmail())){
        return new ApiResponse<>(false , "Email is already used." , null);
    }
        if(userRepository.existsByUserName(request.getUserName())){
            return new ApiResponse<>(false , "UserName is already used." , null);
        }
        ...
        if (request.getRole() == null) {
            ...
            role = roleRepository.findByRoleName(request.getRole());
            if (role == null) {
                return new ApiResponse<>(false, "Role not found in DB.", null);
            }
        }
        user.setRole(role);
